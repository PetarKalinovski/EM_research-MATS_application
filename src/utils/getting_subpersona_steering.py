...
def get_cluster_steering(cluster_id: int, n_clusters: int, layer:int = 20) -> torch.Tensor:
    """
    ...
    """
    try:
        logger.info(f"Loading dataset to compute steering vector for cluster {cluster_id} at layer {layer}...")
        project_root = Path(__file__).resolve().parent.parent.parent
        dataframe = pickle.load(open(project_root / "results" / "final_dataset_with_em_scores_and_clusters.pkl", "rb"))
        logger.info("Dataset loaded successfully.")


        n_clusters='cluster_' + str(n_clusters)
        cluster_data = dataframe[dataframe[n_clusters] == cluster_id]
        logger.info(f"Data filtered for cluster {cluster_id}. Number of samples: {len(cluster_data)}")
        ...
        cluster_base=np.vstack(cluster_data['base_activations_layer_' + str(layer)].to_list())
        logger.info(f"Activation shapes - EM: {cluster_em.shape}, Base: {cluster_base.shape}")

        steering_vector = np.mean(cluster_em, axis=0) - np.mean(cluster_base, axis=0)

        logger.info(f"Steering vector for cluster {cluster_id} at layer {layer} computed.")
